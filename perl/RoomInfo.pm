...
sub generatePossibleNeighboursOnLevel {
	...
	#my $newLevel = shift;
	my ($self, $numberOfRoomsPerLevel, $newLevel) = @_;
	my @result = ();
...
sub generatePossibleNeighbours {
	...
	#my $numberOfRoomsPerLevel = shift;
	my ($self, $numberOfLevels, $numberOfRoomsPerLevel) = @_;
	my @result = ();
	...
	if ($self->{levelNumber} > 0) {
		...
		#	push @result, $neigh;
		#}
		push @result, $self->generatePossibleNeighboursOnLevel($numberOfRoomsPerLevel, $self->{levelNumber} - 1);
	}
