...
# our @ISA = ("Exporter");
# our @EXPORT = qw(new generate navigateLabyrinth); # See page 303.

# ThAw 2014/07/07 : The @EXPORT list is not useful for access control here
# because Labyrinth.pl does not need to import any LabyrinthGenerator functions
# into its namespace; it calls them through a LabyrinthGenerator object.
...
sub roomListContainsRoom {
	...
	#my $roomList = shift;
	#my $room = shift;
	my ($self, $roomList, $room) = @_;

	for my $room2 (@$roomList) {
		return 1 if ($room == $room2); # == is overloaded for RoomInfo objects
	}
